...
const Nivel = ({primero, label, nombre, acumulador }) => {

  const [indicador, useIndicador] = useState(0)

  //actualiza el valor del nivel
  const handleIndicador = e => {
    useIndicador(e.target.value)
    acumulador(e.target.name, e.target.value)
  }

  return(
    <div className={`relative bg-white py-2 px-4 mb-4 shadow w-11/12 md:w-full mx-auto mt-0 ${primero ? 'rounded-b-lg' : 'rounded-lg'}`}>
      {label}
      ...
      <div className="flex items-center">
        ...
        <div className="flex-none my-2 w-2/4">
          ...
          <div className="flex-col">
            <div className="flex justify-center text-xl font-bold text-cyan-800 mb-3">
              { indicador }
            </div>
            <div className="flex">
              <input 
                className="rounded-xl overflow-hidden appearance-none bg-gray-300 h-6 w-full" 
                type="range" 
                min="0" 
                max="10" 
                step="1"
